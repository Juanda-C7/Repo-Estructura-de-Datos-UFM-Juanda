...
class BinarySearchTree:
    ...
    def _delete(self, subtree: Node, value: int) -> Node:
        ...
        if value < subtree.data:
            ...
            # Caso 1 Eliminar un leaf node
            if subtree.left_child is None and subtree.right_child is None:
                return None
            
            # Caso 2 Eliminar un nodo con 1 hijo
            if subtree.left_child is None:
                return subtree.right_child
            elif subtree.right_child is None:
                return subtree.left_child
            
            # Caso 3 Eliminar un nodo con 2 hijos usando in-order predecessor
            predecessor = self.find_max(subtree.left_child)
            subtree.data = predecessor.data
            subtree.left_child = self._delete(subtree.left_child, predecessor.data)
